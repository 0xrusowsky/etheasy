...
impl Component for Frame {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let dark_mode = if self.is_dark_mode() {
            (
                "dark",
                 "transition-all duration-400 ease-in-out absolute h-8 md:h-7 w-1/2 rounded-full bg-gradient-to-r from-teal-600 to-indigo-600 transform translate-x-full"
            )
        } else {
            (
                "",
                "transition-all duration-400 ease-in-out absolute h-8 md:h-7 lg:h-28 w-1/2 rounded-full bg-gray-300 transform translate-x-0"
            )
        };
        ...
        html! {
            ...
            <div class="min-h-screen px-3 bg-gray-100 dark:bg-ruso-dark md:px-0">
            ...
            <div class="w-full max-w-md md:max-w-2xl lg:max-w-4xl 2xl:max-w-6xl 4xl:max-w-8xl">
                <div class="flex items-center justify-between px-4 py-4 -mx-4 border-b border-gray-200 sm:mx-0 sm:px-0">
                    ...
                    <div class="flex items-center ml-6 space-x-6 sm:space-x-10 sm:ml-10">
                        <div class="relative flex w-fit items-center rounded-full">
                            <button class="text-sm font-medium flex items-center gap-2 px-3 md:pl-3 md:pr-3.5 py-3 md:py-1.5 transition-colors relative z-10 text-gray-900 dark:text-gray-400" onclick={ctx.link().callback(|_| Msg::LightMode)}>
                                <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    <circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/>
                                </svg>
                            </button>
                            <button class="text-sm font-medium flex items-center gap-2 px-3 md:pl-3 md:pr-3.5 py-3 md:py-1.5 transition-colors relative z-10 text-gray-500 dark:text-gray-100" onclick={ctx.link().callback(|_| Msg::DarkMode)}>
                                <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <a href="https://github.com/0xrusowsky/devm-toolkit" target="_blank" class="text-gray-600 dark:text-gray-400 transition-colors duration-200 hover:text-gray-900 dark:hover:text-gray-100">
                ...
                <div class="font-mono">
                    <div class="form-control text-sm text-gray-600 dark:text-gray-400 pt-10 pb-2 flex justify-end">
                        <label class="cursor-pointer label">
                        <span>{"Display full EVM words "}</span>
                        <input type="checkbox" checked={self.is_toggled()} class="checkbox checkbox-accent accent-emerald-400" onclick={ctx.link().callback(|_| Msg::Toggle)}/>
                        </label>
                    </div>
                    <div class="subpixel-antialiased text-gray-500 bg-gray-900 dark:bg-ruso-dark_code rounded-md shadow-2xl text-sm">
                        <div class="grid h-full grid-cols-3 p-4">
                            <div class="col-span-1 pt-0 text-sm text-gray-400">
                            ...
                            if self.is_toggled() {
                                <div class="col-span-2 overflow-x-auto text-right text-emerald-400 border-l border-opacity-30">
                                    <p class="pt-0 text-sm text-gray-400">{ "hex: " }</p>
                                    <div> {
                                        for self.hex.split('\n').into_iter().map(|v| {
                                            html!{
